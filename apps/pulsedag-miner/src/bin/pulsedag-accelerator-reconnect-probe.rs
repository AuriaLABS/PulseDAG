...
fn main() -> Result<()> {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "usage: pulsedag-accelerator-reconnect-probe --self-test\n\nRuns a deterministic software-only reconnect/job-refresh/work-redistribution contract. It does not open network sockets, execute CUDA/OpenCL kernels, or claim physical GPU validation."
                );
                return Ok(());
            }
