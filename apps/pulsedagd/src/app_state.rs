...
pub fn new_runtime_stats() -> NodeRuntimeStats {
    let started_at_unix = SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let mut stats = NodeRuntimeStats {
        started_at_unix,
        ..NodeRuntimeStats::default()
    };

    stats.sync_state = "idle".to_string();
    stats.startup_recovery_mode = "unknown".to_string();
    stats.startup_path = "unknown".to_string();
    stats.startup_bootstrap_mode = "unknown".to_string();
    stats.startup_status_summary = "startup status unknown".to_string();
    stats.last_self_audit_ok = true;
    stats.prune_require_snapshot = true;
