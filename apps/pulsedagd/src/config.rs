    ...
    pub fn from_env() -> Self {
        Self {
            ...
            max_future_drift_secs: std::env::var("PULSEDAG_MAX_FUTURE_DRIFT_SECS")
                ...
                .filter(|v| *v > 0)
                .unwrap_or(120),
            mempool_limit: std::env::var("PULSEDAG_MEMPOOL_LIMIT")
                .ok()
                .and_then(|v| v.parse::<usize>().ok())
                .filter(|v| *v > 0)
                .unwrap_or(4096),
            mempool_fee_floor: std::env::var("PULSEDAG_MEMPOOL_FEE_FLOOR")
                .ok()
                .and_then(|v| v.parse::<u64>().ok())
                .unwrap_or(0),
            mempool_ttl_secs: std::env::var("PULSEDAG_MEMPOOL_TTL_SECS")
                .ok()
                .and_then(|v| v.parse::<u64>().ok())
                .filter(|v| *v > 0)
                .unwrap_or(3600),
