                ...
                match event {
                    ...
                    InboundEvent::Block(block) => {
                        ...
                        if let InboundP2pBlockProtocol::ActivatedV2(identity) = inbound_protocol {
                            ...
                            let mut selected_segment_v2_completed = false;
                        ...
                        if matches!(acceptance, BlockAcceptanceResult::MissingParent) {
                            ...
                            if !priority_active && fetch_scheduler.queue_depth() > 0 {
                                let (known, pending) = (
                                    known_hashes_for_scheduler(&guard),
                                    pending_hashes_for_scheduler(&block_requests),
                                );
...
mod tests {
    ...
    fn activated_v2_selected_segment_releases_global_64_cap_for_next_chunk() {
        ...
        for hash in &first {
            assert!(tracker.should_issue_getblock_for_peers(hash, 1, ["peer-a".to_string()],));
            session.requested_hashes.insert(hash.clone());
        }
