...
/// Resolve whether startup may restore the durable activated-v2 P2P runtime.
///
/// Sidecar presence is never an activation signal. Callers must first provide
/// the locally configured protocol capabilities. No capabilities preserves the
/// exact historical startup path; explicit malformed or mismatched capabilities
/// fail closed; only the canonical activated-v2 identity permits strict restore.
