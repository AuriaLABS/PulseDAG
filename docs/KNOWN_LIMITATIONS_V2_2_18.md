# Known Limitations v2.2.18

This document records **current known limitations** for `v2.2.18` and must be read together with release evidence and closeout artifacts.

## Scope and intent

- `v2.2.18` is a **private-testnet RC preparation** milestone.
- `v2.2.18` is **not** a `v2.3.0` readiness declaration.
- `v2.2.18` is **not** a public testnet launch.
...
- GPU mining is optional for `v2.2.18`.
- GPU paths may be scaffold-only where no canonical kernel implementation is present.
- RC acceptance for `v2.2.18` must not depend on GPU availability unless a canonical kernel path and evidence are included in scope.
...
When in doubt, interpret `v2.2.18` outcomes as **readiness signals for additional private rehearsal work**, not as proof of production/public-network readiness.
