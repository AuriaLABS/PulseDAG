...
- Each production CUDA batch uses a dedicated context. On watchdog expiry, ordinary module/memory cleanup is skipped and context teardown owns cleanup, avoiding synchronous `cuMemFree` on known-incomplete work. If `cuCtxDestroy_v2` fails, the loaded CUDA Driver library handle is intentionally retained for process lifetime so a surviving context or deferred driver work cannot outlive unloaded function pointers. CUDA documents context resource cleanup but does not give this slice a hard upper bound for `cuCtxDestroy()` itself.
