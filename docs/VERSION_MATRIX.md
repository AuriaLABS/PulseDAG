# PulseDAG Version Matrix

## Current baseline

| Area | Value |
|---|---|
| VERSION file | `v2.2.18` |
| Cargo workspace version | `2.2.18` |
| Current milestone | v2.2.18 private-testnet RC preparation |
| Current state | **ACTIVE PREFLIGHT / PENDING EVIDENCE** |
| Readiness claims | No v2.3.0 readiness claim; no v3.0 readiness claim |

## v2.2.x progression status

| Version | Scope | Status |
|---|---|---|
| v2.2.17 | API/operator/security hardening closeout | Must be **CLOSED_WITH_EVIDENCE** or **WAIVED_WITH_REASON** before v2.2.18 evidence PASS |
| v2.2.18 | Private-testnet RC preparation and evidence gates | **CURRENT ACTIVE PREFLIGHT/RC PREPARATION** |
| v2.3.0 | Future readiness decision | **FUTURE DECISION ONLY (not readiness claim)** |
