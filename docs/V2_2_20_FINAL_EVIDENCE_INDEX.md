...
| Snapshot restore drill | DOCUMENTED; closeout artifact missing | `docs/SNAPSHOT_RESTORE_DRILL_V2_2_20.md`; expected artifacts `artifacts/v2_2_20_snapshot_restore/<RUN_ID>/evidence_manifest.json`, `evidence.tar.gz`, and `evidence.tar.gz.sha256` | No closeout artifact checksum is attached in-repo for the final decision. | The drill procedure and metadata tests exist, but closeout still needs an attached restore bundle or formal waiver. |
...
3. Snapshot restore has documented automation but no attached final closeout restore artifact/checksum or formal waiver.
