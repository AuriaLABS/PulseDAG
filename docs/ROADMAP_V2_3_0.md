...
This roadmap activates the remaining operational work required for a repeatable multi-host private testnet. It does not authorize a public testnet, a release tag, or a version bump.
...
### Task 08 — Node lifecycle and bootstrap scripts

Add idempotent start/stop/status/upgrade/rollback helpers, persistent directory ownership checks, PID/log handling, bootnode resolution, and safe restart semantics.

### Task 09 — Metrics and dashboard baseline
...
### Task 10 — Operator and incident runbooks
...
### Task 11 — Multi-host private-testnet rehearsal
...
### Task 12 — Version and release decision

Only after Tasks 07–11 pass and a maintainer explicitly approves the release proposal: update `VERSION`/Cargo to `2.3.0`, generate release notes and artifacts, rerun all candidate gates, and record a release decision. This task does not authorize public-testnet launch.

## Completion criteria

v2.3.0 is complete only when Tasks 07–12 are merged, their required checks and operational evidence pass, no unresolved severity-1 consensus/sync/storage/security blocker exists, and the final private-testnet release decision is recorded.
