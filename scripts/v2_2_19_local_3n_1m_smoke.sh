...
write_summary(){
  ...
  for n in a b c; do
    ...
    if [[ -f "$OUT_DIR/endpoints/${n}-readiness.json" ]]; then
      local release_ready p2p_private_ready
      release_ready="$(jq -r '(.data.ready_for_release // .ready_for_release // false)' "$OUT_DIR/endpoints/${n}-readiness.json" 2>/dev/null || echo false)"
      p2p_private_ready="$(jq -r '(.data.p2p_ready_for_private_rehearsal // false)' "$OUT_DIR/endpoints/${n}-readiness.json" 2>/dev/null || echo false)"
      if [[ "$release_ready" == "true" && "$p2p_private_ready" == "true" ]]; then
        ((ready_count+=1))
      fi
    fi
