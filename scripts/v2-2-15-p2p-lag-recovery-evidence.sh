...
CHAIN_ID="${PULSEDAG_REHEARSAL_CHAIN_ID:-pulsedag-lag-recovery-v2-2-15}"
RPC_BASE_PORT="${PULSEDAG_REHEARSAL_RPC_BASE_PORT:-19480}"
P2P_BASE_PORT="${PULSEDAG_REHEARSAL_P2P_BASE_PORT:-19580}"
...
LAG_HOLD_SECS="${PULSEDAG_REHEARSAL_LAG_HOLD_SECS:-20}"
LAG_ADVANCE_BLOCKS="${PULSEDAG_REHEARSAL_LAG_ADVANCE_BLOCKS:-0}"
...
EVIDENCE_ROOT="${PULSEDAG_REHEARSAL_EVIDENCE_ROOT:-$ROOT_DIR/evidence/v2.2.15/p2p-lag-recovery}"
RUN_ID="${PULSEDAG_REHEARSAL_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-${NODE_COUNT}node-lag-recovery}"
...
FAILURES=0
BOOTNODE_1=""
LAG_NODE="${PULSEDAG_REHEARSAL_LAG_NODE:-3}"
...
require_tools() {
  ...
  (( NODE_COUNT >= 3 )) || fatal "PULSEDAG_REHEARSAL_NODE_COUNT must be at least 3 for lag recovery evidence"
  (( LAG_NODE > 1 && LAG_NODE <= NODE_COUNT )) || fatal "PULSEDAG_REHEARSAL_LAG_NODE must be a non-bootstrap node in this topology"
  [[ "$LAG_ADVANCE_BLOCKS" =~ ^[0-9]+$ ]] || fatal "PULSEDAG_REHEARSAL_LAG_ADVANCE_BLOCKS must be numeric"
  [[ "$P2P_MODE" == "libp2p-real" ]] || fatal "lag recovery evidence requires libp2p-real mode; got $P2P_MODE"
}
...
{
  "version": "$VERSION",
  "scenario": "p2p-lag-recovery",
  "node_count": $NODE_COUNT,
  "lag_node": $LAG_NODE,
  "lag_advance_blocks": $LAG_ADVANCE_BLOCKS,
  "chain_id": "$CHAIN_ID",
  "p2p_mode": "$P2P_MODE",
...
write_summary() {
  ...
  {
    echo "PulseDAG $VERSION P2P lag recovery evidence summary"
    echo "run_id=$RUN_ID"
    echo "node_count=$NODE_COUNT"
    echo "lag_node=$(node_name "$LAG_NODE")"
...
if (( LAG_ADVANCE_BLOCKS > 0 )); then
  mine_block 1 baseline-advance
fi
baseline_height="$(node_height 1)"
for ((node=2; node<=NODE_COUNT; node++)); do wait_for_height_at_least "$node" "$baseline_height" || true; done
collect_all_evidence initial

section "Take lagging node offline without deleting DB"
pre_lag_persisted="$(node_persisted_blocks "$LAG_NODE" 2>/dev/null || echo unknown)"
stop_node "$LAG_NODE"
echo "[info] $(node_name "$LAG_NODE") is offline with preserved data dir $(node_dir "$LAG_NODE") (persisted_blocks_before_lag=$pre_lag_persisted)"
sleep "$LAG_HOLD_SECS"
for ((i=1; i<=LAG_ADVANCE_BLOCKS; i++)); do
  mine_block 1 "lag-window-advance-$i"
  sleep 2
done
current_height="$(node_height 1)"
current_tip="$(node_selected_tip 1)"
for ((node=2; node<=NODE_COUNT; node++)); do
  [[ "$node" -eq "$LAG_NODE" ]] && continue
  wait_for_peer_count "$node" 1 || true
  wait_for_height_at_least "$node" "$current_height" || true
done
collect_all_evidence lagging-offline

section "Bring lagging node back without database reset"
[[ -f "$(node_marker "$LAG_NODE")" ]] || fatal "DB preservation marker disappeared before lag recovery restart"
start_node "$LAG_NODE"
wait_for_endpoint "$LAG_NODE" /health
wait_for_endpoint "$LAG_NODE" /p2p/status
wait_for_peer_count "$LAG_NODE" 1 || true
wait_for_height_at_least "$LAG_NODE" "$current_height" || true
wait_for_tip_match "$LAG_NODE" "$current_tip" || true
collect_all_evidence recovered

section "Convergence checks"
if write_summary; then
  pass "lagging node recovered peers, height, tip, chain id, and preserved DB marker"
else
  fail_msg "one or more lag recovery convergence checks failed"
fi

section "Result"
if [[ "$FAILURES" -eq 0 ]]; then
  echo "PASS: $VERSION P2P lag recovery evidence"
  echo "Evidence: $RUN_DIR"
  exit 0
fi

echo "FAIL: $VERSION P2P lag recovery evidence ($FAILURES failing section(s))" >&2
