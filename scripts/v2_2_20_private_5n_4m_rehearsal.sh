...
parse_miner_terminal_evidence(){
  ...
  LOCAL_MINER_SUBMITS_REJECTED_BY_REASON=$(json_array_or_empty "${local_miner_submits_rejected_by_reason:-[]}")
  UNIQUE_SUBMITTED_BLOCK_HASHES=$(json_number_or_zero "${unique_submitted_block_hashes:-0}")
  MINER_EVIDENCE_CONSISTENCY_FAILURES=$(json_array_or_empty "${miner_evidence_consistency_failures:-[]}")
  if [[ "$MINER_EVIDENCE_CONSISTENCY_FAILURES" != "[]" ]]; then
    while IFS= read -r failure; do [[ -n "$failure" ]] && record_fail "MINER_EVIDENCE_CONSISTENCY" "$failure"; done < <(printf '%s' "$MINER_EVIDENCE_CONSISTENCY_FAILURES" | jq -r '.[]' 2>/dev/null || true)
  fi
}
...
collect_final_state(){
  ...
  for i in $(seq 1 "$NODE_COUNT"); do
    ...
    NODE_STORAGE_MEMORY_DIGEST[$i]="$(jq -r '(.data // .) as $d | ($d.checks // [] | map(select(.name == "storage_consistency")) | first // {}) as $s | $s.memory_generation // $d.memory_generation // ""' "$checks_file" 2>/dev/null || echo '')"
    NODE_STORAGE_PERSISTED_DIGEST[$i]="$(jq -r '(.data // .) as $d | ($d.checks // [] | map(select(.name == "storage_consistency")) | first // {}) as $s | $s.storage_generation // $d.storage_generation // ""' "$checks_file" 2>/dev/null || echo '')"
    ...
    NODE_CHECKS_GENERATION[$i]="$(jq -r '(.data // .) as $d | ($d.checks // [] | map(select(.name == "storage_consistency")) | first // {}) as $s | $s.memory_generation // $s.storage_generation // $d.memory_generation // $d.storage_generation // "0"' "$checks_file" 2>/dev/null || echo 0)"
...
write_p2p_convergence_json(){
  jq -n \
    ...
    '{chain_id:$chain_id,version:$version,commit:$commit,primary_failure_class:$failure_class,failure_class:$failure_class,failure_classes:$failure_classes,tip:$tip,accepted_blocks:$accepted_blocks,rejected_blocks:$rejected_blocks,gates:{baseline_5n_1m:$gate_5n_1m,intermediate_5n_2m:$gate_5n_2m,stress_5n_4m:$gate_5n_4m},nodes:$nodes}' \
