...
def compile_root(root: str, evidence_dir: Path, packages_by_id: dict[str, dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for line_number, line in enumerate(result.stdout.splitlines(), start=1):
        try:
            ...
            raise SystemExit(f"non-JSON cargo message for {root} at line {line_number}: {exc}") from exc
    ...
    artifacts.sort(key=lambda item: (item["name"], item["version"], str(item["target_name"]), json.dumps(item["target_kinds"])))
    (evidence_dir / f"{root}-compiler-artifacts.json").write_text(json.dumps(artifacts, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return artifacts
...
def reverse_trees(evidence_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for package in WARNING_PACKAGES:
        result = run([
            "cargo", "tree", "--locked", "--workspace", "--all-targets",
            "--edges", "features", "--invert", package.cargo_spec,
        ], check=False)
...
def main() -> None:
    ...
    (evidence_dir / "cargo-metadata.json").write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    packages_by_id = package_index(metadata)
    locked = {(package["name"], package["version"]) for package in packages_by_id.values()}
    missing = [package.cargo_spec for package in WARNING_PACKAGES if (package.name, package.version) not in locked]
    if missing:
    ...
    for root in roots:
        ...
        for warning in WARNING_PACKAGES:
            warning_reachability[root][warning.name] = [
                artifact for artifact in artifacts
                if artifact["name"] == warning.name and artifact["version"] == warning.version
            ]
    ...
    summary = {
        ...
        "compiled_artifact_counts": {root: len(artifacts) for root, artifacts in compiled.items()},
    }
    (evidence_dir / "warning-reachability-summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    for package in WARNING_PACKAGES:
        ...
        for root in roots:
            ...
            human_lines.append(f"  {root}: compiled={str(bool(matches)).lower()} target_kinds={','.join(kinds) if kinds else '-'}")
