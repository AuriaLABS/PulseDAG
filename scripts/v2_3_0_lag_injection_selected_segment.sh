...
JSON
  for node in n1 n2 n3 n4 n5; do
    cat > "$OUT_DIR/endpoints/${node}-schema-only.json" <<JSON
{"node":"$node","synthetic_schema_evidence":true,"runtime_observed":false}
JSON
    echo "synthetic schema-only placeholder for $node; not closeout eligible" > "$OUT_DIR/logs/${node}.log"
  done
  cat > "$FINAL_TABLE" <<MD
...
run_runtime_evidence() {
  ...
  if [[ ! -r "$HARNESS_PATH" ]]; then
    echo "FATAL: runtime-closeout requires $HARNESS_PATH from PR #739; refusing to fabricate runtime evidence" >&2
    exit 78
  fi
...
archive_evidence() {
  (
    cd "$OUT_DIR"
    tar -czf "$TARBALL" evidence_manifest.json transition_timeline.json gap_timeline.json topology_samples.json selected_segment_counter_summary.json final_convergence_table.md command-log.txt endpoints logs miners pids 2>/dev/null
