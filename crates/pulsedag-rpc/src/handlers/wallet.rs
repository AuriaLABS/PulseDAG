...
fn legacy_wallet_rpc_disabled() -> (StatusCode, Json<ApiResponse<serde_json::Value>>) {
    (
        StatusCode::NOT_FOUND,
        Json(ApiResponse::err(
            "legacy_wallet_rpc_disabled",
            "legacy raw-key wallet RPC is not compiled into this node build",
        )),
    )
}
...
mod legacy {
    ...
    fn sign_transaction_inputs(
        ...
        // Legacy compatibility only: the public key is part of the canonical
        // unsigned transaction and must be attached before signing.
        for input in &mut tx.inputs {
            input.public_key = public_key.clone();
        }
        let signature =
            sign_message(private_key, &signing_message(&tx)).map_err(|e| e.to_string())?;
        for input in &mut tx.inputs {
    ...
    mod tests {
        ...
        fn wallet_transfer_mempool_signs_after_attaching_public_keys() {
            ...
            let signed =
                sign_transaction_inputs(unsigned, &private_key).expect("transaction signs");
...
#[cfg(feature = "legacy-wallet-rpc")]
pub use legacy::{post_wallet_new, post_wallet_sign, post_wallet_transfer};
