...
pub async fn get_sync_verify<S: RpcStateLike>(State(state): State<S>) -> Json<ApiResponse<SyncVerifyData>> {
    ...
    let missing_in_storage = memory_hashes.difference(&persisted_hashes).cloned().collect::<Vec<_>>();
    let missing_in_memory = persisted_hashes.difference(&memory_hashes).cloned().collect::<Vec<_>>();
