...
pub async fn get_node_checks<S: RpcStateLike>(State(state): State<S>) -> Json<ApiResponse<NodeChecksData>> {
    ...
    let checks = vec![
        NodeCheckItem {
            ...
            ok: snapshot_exists,
            detail: if snapshot_exists { "snapshot present".into() } else { "snapshot missing".into() },
        },
        NodeCheckItem {
            ...
            detail: if memory_hashes == persisted_hashes {
                ...
                format!("memory={}, storage={}", memory_hashes.len(), persisted_hashes.len())
            },
        },
        NodeCheckItem {
        ...
        NodeCheckItem {
            name: "p2p_state".into(),
            ok: state.p2p().is_none() || peer_count >= 0,
        ...
        NodeCheckItem {
            ...
            detail: format!("best_height={}, max_block_height={}", chain.dag.best_height, max_block_height),
        },
        NodeCheckItem {
            name: "contracts_namespaces_ready".into(),
            ok: state.storage().contract_namespaces_ready(),
            detail: if state.storage().contract_namespaces_ready() { "contracts namespaces ready".into() } else { "contracts namespaces missing".into() },
        },
    ];
