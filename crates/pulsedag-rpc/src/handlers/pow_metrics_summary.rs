...
pub async fn get_pow_metrics_summary() -> Json<ApiResponse<PowMetricsSummaryData>> {
                ...
                if name.starts_with("pow-") && name.ends_with(".json") && name != "pow-latest.json"
                {
                    if let Ok(bytes) = fs::read(&path) {
                        if let Ok(value) = serde_json::from_slice::<serde_json::Value>(&bytes) {
                            if let Some(d) =
                                value.get("suggested_difficulty").and_then(|v| v.as_u64())
                            {
                                difficulties.push(d);
                            }
                            if let Some(i) = value
    ...
    let avg_suggested_difficulty = if difficulties.is_empty() {
        0.0
    } else {
        difficulties.iter().copied().sum::<u64>() as f64 / difficulties.len() as f64
    };
    ...
    let avg_block_interval_secs = if intervals.is_empty() {
        0.0
    } else {
        intervals.iter().copied().sum::<u64>() as f64 / intervals.len() as f64
    };
