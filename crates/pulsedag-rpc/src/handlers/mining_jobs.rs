...
use crate::{api::{ApiResponse, ClaimMiningJobRequest, RpcStateLike, SubmitMiningJobRequest}, handlers::pow_metrics::PowMetricsData, handlers::mining_pool::load_worker_config, handlers::mining_accounting::credit_block};
use pulsedag_core::{accept_block, build_candidate_block, build_coinbase_transaction, dev_pow_accepts, dev_recommended_difficulty_for_chain, dev_target_u64, AcceptSource};
...
pub async fn post_claim_mining_job<S: RpcStateLike>(State(state): State<S>, Json(req): Json<ClaimMiningJobRequest>) -> Json<ApiResponse<ClaimMiningJobData>> {
    ...
    let block = build_candidate_block(parents, height, difficulty as u32, txs);
    let now = unix_now();
    let job_id = format!("job-{}-{}", req.worker_id, now);
    let expires_at_unix = now + 30;
    let share_difficulty = load_worker_config(&req.worker_id).map(|c| c.share_difficulty).unwrap_or(1);
    ...
    let metrics_hint = PowMetricsData {
        ...
        target_u64: dev_target_u64(difficulty as u64),
...
pub async fn post_submit_mining_job<S: RpcStateLike>(State(state): State<S>, Json(req): Json<SubmitMiningJobRequest>) -> Json<ApiResponse<SubmitMiningJobData>> {
    let Some(mut record) = load_job(&req.job_id) else {
        return Json(ApiResponse::err("UNKNOWN_JOB", format!("job not found: {}", req.job_id)));
    };
    if record.worker_id != req.worker_id {
        return Json(ApiResponse::err("JOB_WORKER_MISMATCH", "job does not belong to this worker".to_string()));
    }
    if record.submitted {
        return Json(ApiResponse::err("JOB_ALREADY_SUBMITTED", "job already submitted".to_string()));
    }
    if unix_now() > record.expires_at_unix {
        return Json(ApiResponse::err("JOB_EXPIRED", "job expired before submission".to_string()));
    }
    ...
    if req.block.header.height <= chain.dag.best_height {
        return Json(ApiResponse::err("STALE_JOB", format!("stale job: current best height is {} and submitted block height is {}", chain.dag.best_height, req.block.header.height)));
    }
...
fn persist_job(record: &MiningJobRecord) -> std::io::Result<()> {
    ...
    fs::write(dir.join(format!("{}.json", sanitize(&record.job_id))), serde_json::to_vec_pretty(record).unwrap_or_default())
}
...
pub async fn post_cleanup_mining_jobs() -> Json<ApiResponse<CleanupMiningJobsData>> {
    ...
    Json(ApiResponse::ok(CleanupMiningJobsData { removed_jobs: removed_job_ids.len(), removed_job_ids }))
}
