...
pub async fn post_prune_chain<S: RpcStateLike>(
    ...
    let pre_prune_rebuilt = match rebuild_state_from_blocks(chain_id.clone(), pre_prune_blocks) {
        Ok(v) => v,
        Err(e) => {
    ...
    let rebuilt = match rebuild_state_from_blocks(chain_id, post_prune_blocks) {
        ...
        Err(e) => {
            let _ = state.storage().append_runtime_event(
                ...
                &format!(
                    "replay postcheck failed after prune at keep_from_height {}: {}",
                    keep_from_height, e
                ),
            );
            return Json(ApiResponse::err(
    ...
    let _ = state.storage().append_runtime_event(
        ...
        "prune_manual",
        &format!(
            "manual prune removed {} blocks below {}; replay verified (pre {} / post {})",
            pruned_block_count,
            keep_from_height,
            pre_prune_rebuilt.dag.best_height,
