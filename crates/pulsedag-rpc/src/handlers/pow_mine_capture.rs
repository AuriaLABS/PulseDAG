...
pub async fn post_pow_mine_capture<S: RpcStateLike>(State(state): State<S>, Json(req): Json<MineRequest>) -> Json<ApiResponse<PowMineCaptureData>> {
    ...
    match accept_block(block.clone(), &mut chain, AcceptSource::LocalMining) {
        Ok(_) => {
            ...
            let snapshot_path = snapshot_dir.join(format!("pow-mine-{}.json", SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0)));
