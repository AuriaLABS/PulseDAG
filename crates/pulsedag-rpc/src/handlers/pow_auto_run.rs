...
use pulsedag_core::{accept_block, dev_mine_header, dev_pow_accepts, dev_recommended_difficulty_for_chain, dev_target_u64, mining::{build_candidate_block, build_coinbase_transaction}, AcceptSource};
...
pub async fn post_pow_auto_run<S: RpcStateLike>(State(state): State<S>, Json(req): Json<PowAutoRunRequest>) -> Json<ApiResponse<PowAutoRunData>> {
    ...
    for _ in 0..rounds {
        ...
        let txs = vec![build_coinbase_transaction(&req.miner_address, reward, height)];
        let mut block = build_candidate_block(parents, height, difficulty as u32, txs);
        let (mined_header, _accepted, pow_tries, _pow_hash) = dev_mine_header(block.header.clone(), max_tries);
        ...
        match accept_block(block.clone(), &mut chain, AcceptSource::LocalMining) {
            Ok(_) => {
                ...
                let snapshot_path = snapshot_dir.join(format!("pow-auto-{}-{}.json", height, SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0)));
                ...
                let _ = fs::write(&snapshot_path, serde_json::to_vec_pretty(&payload).unwrap_or_default());
    ...
    Json(ApiResponse::ok(PowAutoRunData { rounds, mined, items }))
}
