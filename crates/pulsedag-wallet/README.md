...
When `tx-preview` or `tx-sign` encounters a selected outpoint that is already reserved by a pending transaction, the command exits non-zero and emits one JSON line to stderr with the stable shape `{"ok":false,"error":{"code":"PENDING_UTXO_RESERVED","message":"selected outpoint is reserved by a pending transaction","txid":"<canonical-lowercase-32-byte-hex>","index":<u32>}}`. Other CLI failures retain the existing human-readable stderr format.
...
Wallet unlock sessions are fail-closed across suspend/resume: if wall time resumes at or beyond the unlock deadline, the session is locked before any further private-key or deterministic-child operation. Backward wall-clock discontinuities are treated the same way; callers must unlock again after either condition.
...
`confirmed` records are terminal reservation state rather than permanent transaction history. The exact confirmed txid remains available for its terminal same-txid check until a genuinely new reservation is appended; when that happens, older `confirmed` entries may be pruned before the new pending record is stored. Active/reserving states are never pruned by this maintenance step.
