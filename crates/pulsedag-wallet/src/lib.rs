...
pub fn build_transaction(from: &str, to: &str, amount: u64, fee: u64, available_utxos: &[Utxo], nonce: u64) -> Result<BuildTxResponse, PulseError> {
    let target = amount.checked_add(fee).ok_or_else(|| PulseError::InvalidTransaction("amount overflow".into()))?;
    let (selected, total_input) = select_utxos(available_utxos, target)?;
    let change = total_input - target;
    let inputs = selected.iter().map(|u| TxInput { previous_output: u.outpoint.clone(), public_key: String::new(), signature: String::new() }).collect::<Vec<_>>();
    let mut outputs = vec![TxOutput { address: to.to_string(), amount }];
    if change > 0 {
        outputs.push(TxOutput { address: from.to_string(), amount: change });
    }
    let mut tx = Transaction { txid: String::new(), version: 1, inputs, outputs, fee, nonce };
    let message = signing_message(&tx);
    tx.txid = compute_txid(&tx);
    Ok(BuildTxResponse {
        transaction: tx,
        selected_utxos: selected.iter().map(|u| SelectedUtxo { outpoint: u.outpoint.clone(), amount: u.amount }).collect(),
        total_input,
        change,
