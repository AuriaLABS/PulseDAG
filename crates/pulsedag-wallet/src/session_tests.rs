...
fn fixture(label: &str) -> (PathBuf, WalletKeystoreFile, String) {
    ...
    fs::create_dir(&dir).expect("create session fixture directory");
    ...
    .expect("encrypt session fixture");
    let file = WalletKeystoreFile::try_acquire(&path).expect("lock session fixture");
    file.create_new(&envelope).expect("persist session fixture");
    (dir, file, address)
}
...
#[test]
fn session_starts_locked_unlocks_and_manual_lock_destroys_active_state() {
    let (dir, file, address) = fixture("manual-lock");
    let policy = WalletUnlockPolicy::new(Duration::from_secs(5), 3, Duration::from_secs(1))
        .expect("valid policy");
    let mut session = WalletSession::new(policy).expect("create session");
...
fn expiry_worker_relocks_without_polling_and_use_does_not_extend_deadline() {
    ...
    let policy = WalletUnlockPolicy::new(
    ...
    session
        ...
        .expect("secret remains available before deadline");
    thread::sleep(Duration::from_millis(180));

    {
        let state = lock_state(&session.shared);
        assert!(state.unlocked.is_none(), "expiry worker must clear the secret state");
    }
    assert_eq!(session.status().lock_state, WalletSessionLockState::Locked);
    cleanup(dir, file, session);
}
...
#[test]
fn failed_unlocks_trigger_local_rate_limit_until_lockout_expires() {
    ...
    session
        .record_authentication_failure(Instant::now())
