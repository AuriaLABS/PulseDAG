...
impl WalletPendingJournal {
    ...
    pub fn mark_submission_started(&mut self, final_txid: &str) -> Result<(), WalletPendingError> {
        self.validate()?;
        validate_txid(final_txid)?;
        let entry = self
            .entries
