...
pub fn validate_transaction(tx: &Transaction, state: &ChainState) -> Result<(), PulseError> {
    ...
    let total_input = tx.inputs.iter().try_fold(0_u64, |acc, input| {
        let utxo = state.utxo.utxos.get(&input.previous_output).ok_or(PulseError::UtxoNotFound)?;
        if state.mempool.spent_outpoints.contains(&input.previous_output) {
...
pub fn validate_block(block: &Block, state: &ChainState) -> Result<(), PulseError> {
    ...
    if block.header.timestamp == 0 {
        return Err(PulseError::InvalidBlock("timestamp must be greater than zero".into()));
    }
    let now = current_ts();
    let max_future = crate::dev_max_future_drift_secs();
    if block.header.timestamp > now.saturating_add(max_future) {
        return Err(PulseError::InvalidBlock(format!("timestamp too far in the future: {} > {} + {}", block.header.timestamp, now, max_future)));
    }
    ...
    for parent in &block.header.parents {
        ...
        let parent_block = state.dag.blocks.get(parent).ok_or_else(|| PulseError::InvalidBlock(format!("missing parent {parent}")))?;
    ...
    if block.header.height != expected_height {
        return Err(PulseError::InvalidBlock(format!("invalid height {}, expected {}", block.header.height, expected_height)));
    }
    if block.header.timestamp < newest_parent_timestamp {
        return Err(PulseError::InvalidBlock(format!("timestamp {} is older than newest parent {}", block.header.timestamp, newest_parent_timestamp)));
    }
    ...
    if block.transactions.iter().skip(1).any(is_coinbase) {
        return Err(PulseError::InvalidBlock("multiple coinbase transactions".into()));
    }
    let mut seen_txids = BTreeSet::new();
    for tx in &block.transactions {
        if !seen_txids.insert(tx.txid.clone()) {
            return Err(PulseError::InvalidBlock("duplicate transaction in block".into()));
        }
    }
