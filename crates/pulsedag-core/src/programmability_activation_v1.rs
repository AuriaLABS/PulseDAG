...
use crate::{
    contract_state_v1::{CONTRACT_STATE_TRANSITION_VERSION_V1, PROOF_EVIDENCE_POLICY_VERSION_V1},
    contract_v3::{
...
impl ProgrammabilityActivationContractV1 {
        ...
        match self.mode {
            ProgrammabilityActivationModeV1::Inactive => {
                if self.protocol_upgrade_authorization_commitment
                    != INACTIVE_PROTOCOL_UPGRADE_AUTHORIZATION_COMMITMENT
                {
                    return Err(ProgrammabilityActivationV1Error::InactiveAuthorizationMustBeZero);
                }
            }
    ...
    pub fn validate_against(
        ...
        self.validate_shape()?;
        let expected =
            FrozenActivationBindingsV1::derive(expected_identity, expected_base_protocol)?;
...
impl FrozenActivationBindingsV1 {
    fn derive(
        ...
        if identity.base_protocol_fingerprint != base_protocol_fingerprint {
            return Err(ProgrammabilityActivationV1Error::ProgrammabilityBaseProtocolMismatch);
        }
...
pub fn canonical_programmability_activation_contract_bytes_v1(
    ...
    encode_len_prefixed(&mut out, PROGRAMMABILITY_ACTIVATION_CONTRACT_DOMAIN_V1)?;
...
mod tests {
    ...
    fn programmability_identity() -> ProgrammabilityActivationIdentity {
        ...
        ProgrammabilityActivationIdentity {
            ...
            base_protocol_fingerprint: protocol_activation_identity_fingerprint_bytes(
                &base_protocol,
            )
            .unwrap(),
        }
    ...
    fn inactive_contract() -> ProgrammabilityActivationContractV1 {
        ProgrammabilityActivationContractV1::inactive(&programmability_identity(), &base_protocol())
            .unwrap()
    }
    ...
    fn inactive_never_authorizes_even_when_compile_and_runtime_are_open() {
        ...
        let protocol = base_protocol();
        let contract = ProgrammabilityActivationContractV1::inactive(&identity, &protocol).unwrap();

        assert_eq!(
            authorize_with_gates(&contract, &identity, &protocol, true, true, [0x44; 32],),
    ...
    fn protocol_upgrade_requires_independent_trusted_authorization() {
        ...
        assert!(
            authorize_with_gates(&contract, &identity, &protocol, true, true, authorization,)
                .is_ok()
        );
        assert_eq!(
            authorize_with_gates(&contract, &identity, &protocol, true, true, [0; 32],),
            Err(ProgrammabilityActivationV1Error::TrustedAuthorizationMissing)
        );
        assert_eq!(
            authorize_with_gates(&contract, &identity, &protocol, true, true, [0x45; 32],),
    ...
    fn compile_and_runtime_gates_are_both_required_but_not_sufficient() {
        ...
        assert_eq!(
            authorize_with_gates(&contract, &identity, &protocol, false, true, authorization,),
            Err(ProgrammabilityActivationV1Error::CompileGateClosed)
        );
        assert_eq!(
            authorize_with_gates(&contract, &identity, &protocol, true, false, authorization,),
    ...
    fn inactive_and_protocol_upgrade_authorization_shapes_are_strict() {
        ...
        let protocol = base_protocol();
        assert_eq!(
            ProgrammabilityActivationContractV1::protocol_upgrade(&identity, &protocol, [0; 32],),
            Err(ProgrammabilityActivationV1Error::ProtocolUpgradeAuthorizationMustBeNonzero)
        );
    }
    ...
    fn canonical_replay_and_strict_serde_are_stable() {
        ...
        let decoded: ProgrammabilityActivationContractV1 = serde_json::from_str(&encoded).unwrap();
        ...
        assert!(
            serde_json::from_value::<ProgrammabilityActivationContractV1>(unknown_field).is_err()
        );

        let mut unknown_mode = serde_json::to_value(&contract).unwrap();
        unknown_mode["mode"] = serde_json::json!("future_mode");
        assert!(
            serde_json::from_value::<ProgrammabilityActivationContractV1>(unknown_mode).is_err()
        );
        ...
        assert!(matches!(
            ...
            Err(ProgrammabilityActivationV1Error::UnsupportedActivationContractVersion(_))
        ));
    }
    ...
    fn consensus_fields_are_fingerprint_bound_or_fail_closed() {
        ...
        let protocol = base_protocol();
        let upgrade_a =
            ProgrammabilityActivationContractV1::protocol_upgrade(&identity, &protocol, [0x44; 32])
                .unwrap();
        let upgrade_b =
            ProgrammabilityActivationContractV1::protocol_upgrade(&identity, &protocol, [0x45; 32])
                .unwrap();

        assert_ne!(upgrade_a.fingerprint().unwrap(), base_fingerprint);
        assert_ne!(
            upgrade_b.fingerprint().unwrap(),
            upgrade_a.fingerprint().unwrap()
        );
    ...
    fn rejection_codes_are_stable() {
        ...
        assert_eq!(
            ProgrammabilityActivationV1Error::AuthorizationCommitmentMismatch.rejection_code(),
            14
        );
        assert_eq!(
