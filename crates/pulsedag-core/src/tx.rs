...
use crate::{errors::PulseError, state::ChainState, types::{Address, OutPoint, Transaction, TxOutput}};
...
pub fn signing_message(tx: &Transaction) -> Vec<u8> {
    ...
    let inputs = tx.inputs.iter().map(|i| &i.previous_output).collect::<Vec<_>>();
...
pub fn verify_transaction_signatures(tx: &Transaction, state: &ChainState) -> Result<(), PulseError> {
    ...
    for input in &tx.inputs {
        ...
        let pk_arr: [u8; 32] = pk_bytes.try_into().map_err(|_| PulseError::InvalidSignature)?;
        let sig_arr: [u8; 64] = sig_bytes.try_into().map_err(|_| PulseError::InvalidSignature)?;

        let verifying_key = VerifyingKey::from_bytes(&pk_arr).map_err(|_| PulseError::InvalidSignature)?;
        let signature = Signature::from_bytes(&sig_arr);
        verifying_key.verify(&message, &signature).map_err(|_| PulseError::InvalidSignature)?;
    }
