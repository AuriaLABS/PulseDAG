use crate::{errors::PulseError, mining::is_coinbase, state::ChainState, types::{Block, OutPoint, Transaction, Utxo}};

pub fn apply_transaction(tx: &Transaction, state: &mut ChainState, height: u64) -> Result<(), PulseError> {
    if !is_coinbase(tx) {
        for input in &tx.inputs {
            let spent = state.utxo.utxos.remove(&input.previous_output).ok_or(PulseError::UtxoNotFound)?;
    ...
    for (index, output) in tx.outputs.iter().enumerate() {
        let outpoint = OutPoint { txid: tx.txid.clone(), index: index as u32 };
        let utxo = Utxo {
        ...
        state.utxo.address_index.entry(output.address.clone()).or_default().push(outpoint);
    }
...
pub fn apply_block(block: &Block, state: &mut ChainState) -> Result<(), PulseError> {
    ...
    for parent in &block.header.parents {
        ...
        state.dag.children.entry(parent.clone()).or_default().push(block.hash.clone());
    }
