...
///
/// The claim intentionally reuses the active chain-bound v2 transaction
/// serialization instead of stealing transaction version 3, which is reserved
/// for the hybrid PQC transaction format. The zero-valued claim commits the
/// beneficiary and nonce while the monetary amount remains state-derived.
pub fn compute_reward_claim_txid_v3(
    tx: &Transaction,
    chain_id: &str,
) -> Result<Hash, RewardSettlementV3Error> {
    validate_reward_claim_shape_v3(tx, chain_id)?;
    compute_txid_v2(tx, chain_id)
        .map_err(|error| RewardSettlementV3Error::InvalidRewardClaim(error.to_string()))
}
...
mod tests {
    ...
    #[test]
    fn reward_claim_uses_v2_domain_and_commits_beneficiary_but_not_amount() {
