use pulsedag_core::{
    ...
    accept_transaction_with_mempool_policy_v3_for_protocol, canonical_resource_survivors_v1,
    genesis::init_chain_state,
    mempool_resource_rejection_code_from_reason_v1, normalize_production_mempool_resources_v1,
...
fn restored_capacity_normalization_is_insertion_order_deterministic() {
    ...
    assert_eq!(
        canonical_resource_survivors_v1(&a),
        canonical_resource_survivors_v1(&b)
    );
...
fn orphan_expiry_uses_same_1440_height_boundary_and_missing_age_migrates_conservatively() {
    ...
    assert_eq!(
        boundary.expired_orphan_txids,
        vec!["aged-orphan".to_string()]
    );
    assert!(!state
        .mempool
        .orphan_transactions
        .contains_key("aged-orphan"));
    assert!(state
        .mempool
        .orphan_transactions
        .contains_key("migrated-orphan"));
}
