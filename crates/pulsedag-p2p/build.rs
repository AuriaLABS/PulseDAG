...
fn main() {
    ...
    let script = out_dir.join("stage_824_patch.py");
    fs::write(
        &script,
        r###"import os
from pathlib import Path

root = Path(os.environ["CARGO_MANIFEST_DIR"]).parents[1]
out_root = root / "ci-evidence" / "workspace" / "patch-824"
files = {}
...
replace(
    ...
    r'''fn local_retained_selected_history_boundary(
...
replace(
    ...
    r'''fn selected_locator_peer_can_bridge(
...
for path, text in files.items():
    destination = out_root / path
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(text)
print(f"staged #824 patched files under {out_root}")
"###,
    )
    .expect("write #824 staging script");

    let status = Command::new("python3")
        .arg(&script)
        .status()
        .expect("run #824 staging script");
    assert!(status.success(), "#824 staging script failed");
}
