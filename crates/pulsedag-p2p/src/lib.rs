...
use crate::live_compact_relay_v1::{
    authorized_compact_relay_from_tip, dispatch_reconstructed_compact_block_v1,
    encode_compact_relay_for_transport, validate_compact_relay_send,
};
...
const TIP_INVENTORY_TTL_SECS: u64 = 10 * 60;

#[derive(Debug, Clone)]
struct RemoteTipInventoryEntry {
    status: RemoteSelectedTipStatus,
    last_seen_unix: u64,
}
...
pub struct P2pStatus {
    ...
    pub getblock_requests_sent_total: u64,
    pub getblock_responses_correlated_total: u64,
    pub peer_addressed_getblock_sent_total: u64,
    pub peer_addressed_getblock_response_total: u64,
    pub generic_getblock_response_total: u64,
    pub wrong_peer_response_total: u64,
    pub wrong_hash_response_total: u64,
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct OutstandingGetBlockRequest {
    request_id: String,
    target_peer_id: String,
    block_hash: PulseHash,
    completed: bool,
}
...
pub trait P2pHandle: Send + Sync {
    ...
    fn submit_reconstructed_compact_block_v1(
        &self,
        _peer_id: &str,
        _block: &Block,
    ) -> Result<(), PulseError> {
        Err(PulseError::Internal(
            "compact-relay reconstructed block submission is not supported by this p2p handle"
                .into(),
        ))
    }
    fn send_compact_relay_full_block_fallback_v1(
        &self,
        request_hash: &PulseHash,
        block: Option<&Block>,
    ) -> Result<(), PulseError> {
        self.send_block_data(Some(request_hash), block)
    }
    ...
    fn send_block_data(
        ...
        self.send_block_data_with_request_id(None, request_hash, block)
    }
    fn send_block_data_with_request_id(
        &self,
...
struct InnerState {
    ...
    peer_zero_reconnect_pending: HashSet<String>,
    ...
    getblock_requests_sent_total: u64,
    getblock_responses_correlated_total: u64,
    generic_getblock_response_total: u64,
    wrong_peer_response_total: u64,
    wrong_hash_response_total: u64,
    outstanding_getblock_requests: HashMap<String, OutstandingGetBlockRequest>,
...
impl P2pHandle for MemoryP2pHandle {
    ...
    fn update_tip_inventory(&self, inventory: TipInventoryStatus) -> Result<(), PulseError> {
        let mut inner = self
            ...
            .lock()
            .map_err(|_| PulseError::Internal("p2p lock poisoned".into()))?;
        next_local_tip_inventory(&mut inner, inventory);
        Ok(())
    }
    ...
    fn request_block_broadcast(&self, _hash: &PulseHash) -> Result<(), PulseError> {
        ...
        inner.getblock_requests_sent_total = inner.getblock_requests_sent_total.saturating_add(1);
    ...
    fn request_block_from(
        ...
        inner.outstanding_getblock_requests.insert(
            request_id.clone(),
    ...
    fn status(&self) -> Result<P2pStatus, PulseError> {
        ...
        Ok(P2pStatus {
            ...
            getblock_requests_received_by_connection_direction: inner
                ...
                .clone(),
            getblock_requests_sent_total: inner.getblock_requests_sent_total,
            getblock_responses_correlated_total: inner.getblock_responses_correlated_total,
            ...
            generic_getblock_response_total: inner.generic_getblock_response_total,
            wrong_peer_response_total: inner.wrong_peer_response_total,
            wrong_hash_response_total: inner.wrong_hash_response_total,
...
impl Libp2pHandle {
    pub fn new(
        ...
        let mut state = InnerState {
            ...
            peer_zero_reconnect_pending: HashSet::new(),
...
fn next_local_tip_inventory(state: &mut InnerState, mut inventory: TipInventoryStatus) {
    state.local_tip_inventory_generation = state.local_tip_inventory_generation.saturating_add(1);
    inventory.inventory_generation = state.local_tip_inventory_generation;
    if inventory.observed_at_unix == 0 {
        inventory.observed_at_unix = now_unix();
    }
    state.local_tip_inventory = Some(inventory);
}

fn current_tip_inventory(state: &InnerState, chain_id: &str) -> Option<TipInventoryStatus> {
    state.local_tip_inventory.as_ref().map(|inventory| {
        let mut inventory = inventory.clone();
        inventory.chain_id = chain_id.to_string();
        inventory
    })
}
...
fn note_remote_tip_inventory(
    ...
    *state
    ...
    let selected_height = inventory.selected_height.unwrap_or(0);
    let replacement_is_fresher = state
        .remote_selected_tip_inventory
        .get(peer_id)
        .map(|current| {
            inventory.inventory_generation > current.status.inventory_generation
                || (inventory.inventory_generation == current.status.inventory_generation
                    && inventory.observed_at_unix > current.status.observed_at_unix)
        })
        .unwrap_or(true);
...
fn remote_tip_inventory_snapshot(state: &mut InnerState) -> Vec<RemoteSelectedTipStatus> {
    prune_remote_tip_inventory(state, now_unix());
    state
        .remote_selected_tip_inventory
        .values()
        .map(|entry| {
...
fn enqueue_outbound_message(
    ...
    match msg {
        ...
        OutboundMessage::CompactRelayReconstructedBlock { peer_id, block } => {
            queue
                .blocks
                .push_back(OutboundMessage::CompactRelayReconstructedBlock { peer_id, block });
        }
...
fn enforce_connectivity_aware_cooldown_floor(state: &mut InnerState, now: u64) {
    ...
    if active_count == 0 && (!state.bootnodes_configured.is_empty() || !state.peer_book.is_empty())
    {
        ...
        let completed_zero_peer_attempts = state
            .peer_zero_reconnect_pending
            .iter()
            .filter(|peer| state.active_connections.get(*peer).copied().unwrap_or(0) > 0)
            .cloned()
            .collect::<Vec<_>>();
        for peer in completed_zero_peer_attempts {
            state.peer_zero_reconnect_pending.remove(&peer);
            if state.peer_zero_reconnect_attempt_total > state.peer_zero_reconnect_success_total {
                state.peer_zero_reconnect_success_total =
                    state.peer_zero_reconnect_success_total.saturating_add(1);
            }
        }
        if state.peer_zero_reconnect_pending.is_empty() {
    ...
    match msg {
        ...
        NetworkMessage::GetBlock {
            ...
            if requested_peer_id.as_ref().is_none_or(|peer| {
                ...
                let _ = inbound_tx.send(InboundEvent::GetBlock { hash, request_id });
            }
        }
        NetworkMessage::BlockData {
            ...
            if let Some(block) = block {
                ...
                if let Ok(mut guard) = inner.lock() {
                    guard.blocks_received = guard.blocks_received.saturating_add(1);
                    let mut correlated_request = false;
                    if let Some(request_id) = request_id.as_ref() {
                        if let Some(outstanding) =
                            guard.outstanding_getblock_requests.get_mut(request_id)
                        {
        ...
        tokio::select! {
            Some(msg) = outbound_rx.recv() => {
                ...
                while let Some(msg) = pop_outbound_message(&inner, &mut outbound_queue) {
                    let msg = match msg {
                        OutboundMessage::CompactRelayReconstructedBlock { peer_id, block } => {
                            if let Err(error) = dispatch_reconstructed_compact_block_v1(
                                &cfg.chain_id,
                                &peer_id,
                                block,
                                &inner,
                                ...
                                note_swarm_event(
                                    &inner,
                                    format!("compact-relay-reinject-failed:{peer_id}:{error}"),
                                );
                            }
                            continue;
                        }
                        OutboundMessage::CompactRelay {
                            ...
                            match encode_compact_relay_for_transport(
                                &inner,
                                &cfg.chain_id,
                                &peer_id,
                                &compact_relay,
                                ...
                                Err(error) => {
                                    note_swarm_event(
                                        &inner,
                                        format!(
                                            "compact-relay-send-failed:{peer_id}:encode:{error}"
                                        ),
                                    );
                    ...
                    OutboundMessage::CompactRelay { .. } => {
                        unreachable!("compact relay sends are consumed before generic publish")
                    }
                    OutboundMessage::GetBlockHeaders(hashes) => {
                        let topic_name = format!("{}-sync", cfg.chain_id);
                    ...
                    OutboundMessage::GetBlock(hash) => {
                        ...
                        let wire = serde_json::to_vec(&NetworkMessage::GetBlock { chain_id: cfg.chain_id.clone(), hash, request_id: None, requesting_peer_id: None, requested_peer_id: None, request_kind: Some("generic".into()) });
                    ...
                    OutboundMessage::GetBlockFrom { peer_id, hash, request_id } => {
                        ...
                        let wire = serde_json::to_vec(&NetworkMessage::GetBlock { chain_id: cfg.chain_id.clone(), hash, request_id: Some(request_id), requesting_peer_id: None, requested_peer_id: Some(peer_id), request_kind: Some("generic".into()) });
                        (wire, topic_name, "get-block-from", message_id)
                    }
                    OutboundMessage::BlockData { block, request_id, request_hash } => {
                        let topic_name = format!("{}-sync", cfg.chain_id);
                        ...
                        let wire = serde_json::to_vec(&NetworkMessage::BlockData { chain_id: cfg.chain_id.clone(), block, request_id, request_hash });
...
fn record_bootnode_reconnect_schedule(
    ...
    inner
        ...
        .map(|mut guard| {
            ...
            let zero_peer_isolated = mode_connected_peers_are_real_network(&guard.mode)
                && guard.connected_peers.is_empty()
                && guard.active_connections.is_empty()
                && !guard.bootnodes_configured.is_empty()
                && !guard.pending_bootnode_dials.contains(&peer_key)
                && !guard.peer_zero_reconnect_pending.contains(&peer_key);
            if zero_peer_isolated {
                guard.peer_zero_reconnect_pending.insert(peer_key);
                guard.peer_zero_reconnect_attempt_total =
                    guard.peer_zero_reconnect_attempt_total.saturating_add(1);
                guard.last_peer_reconnect_blocked_reason = None;
            }
...
fn handle_connection_established(
    ...
    if let Ok(mut guard) = inner.lock() {
        ...
        if is_bootnode {
            ...
            guard.bootnode_next_redial_at.remove(&peer_key);
            guard
                .bootnode_redial_backoff_secs
                .insert(peer_key.clone(), 1);
        }
        let count = guard
            .active_connections
        ...
        *count = count.saturating_add(1);
        ...
        // A libp2p connection-established event can be transient; defer zero-peer
        // recovery success accounting until the next fresh status/cooldown pass
        // observes the peer as still connected.
        if is_bootnode
            && !guard
        ...
        *guard
        ...
        if remaining_count == 0 {
            ...
            if is_bootnode {
                ...
                let isolated_after_close = mode_connected_peers_are_real_network(&guard.mode)
                    && guard.connected_peers.is_empty()
                    && guard.active_connections.is_empty()
                    && !guard.bootnodes_configured.is_empty();
                let next_at = if isolated_after_close {
                    guard.peer_zero_since_unix.get_or_insert(now);
                    now
                } else {
                    now.saturating_add(current)
                };
...
fn handle_outgoing_connection_error(
    ...
    if let Ok(mut guard) = inner.lock() {
        ...
        if is_bootnode {
            ...
            let peer_key = peer_id.to_string();
            guard.pending_bootnode_dials.remove(&peer_key);
            guard.peer_zero_reconnect_pending.remove(&peer_key);
            guard.last_peer_reconnect_blocked_reason = Some("dial_failed".to_string());
            guard.pending_bootnode_dial_started_at.remove(&peer_key);
            let now = now_unix();
            let current = guard
                .bootnode_redial_backoff_secs
            ...
            guard
                ...
                .insert(peer_key.clone(), now.saturating_add(current));
            guard
                .bootnode_redial_backoff_secs
                .insert(peer_key.clone(), next);
                                ...
                                Ok(bytes) => {
                                        ...
                                        swarm.behaviour_mut().gossipsub.publish(topic, bytes)
                                    {
                                        note_swarm_event(
                                            &inner,
                                            format!(
                                                "compact-relay-send-failed:{peer_id}:publish:{error}"
                                            ),
                                        );
                                        let _ = inbound_tx.send(
                                            InboundEvent::CompactRelaySendFailed {
                ...
                match event {
                    ...
                    SwarmEvent::ConnectionClosed { peer_id, endpoint, cause, .. } => {
                        ...
                        if should_mark_disconnected {
                            let peer_key = peer_id.to_string();
                            register_peer_result(&inner, &peer_key, false);
                            let _ = inbound_tx.send(InboundEvent::PeerDisconnected(peer_key));
                        }
                    }
                    ...
                    if pending_bootnode_dials.contains(peer_id) {
                        ...
                        if stale_pending {
                            ...
                            if let Ok(mut guard) = inner.lock() {
                                ...
                                guard.peer_zero_reconnect_pending.remove(&peer_id.to_string());
                            ...
                            if let Ok(mut guard) = inner.lock() {
                                guard.last_peer_reconnect_blocked_reason = Some("dial_in_progress".to_string());
                            }
...
impl Libp2pHandle {
    fn queue_sync_message_with_policy(
        &self,
        msg: OutboundMessage,
        kind: &str,
        fail_on_backpressure: bool,
    ) -> Result<(), PulseError> {
        let block_lane = matches!(
            &msg,
            OutboundMessage::InvBlock(_)
                | OutboundMessage::CompactRelayReconstructedBlock { .. }
                | OutboundMessage::GetHeaders { .. }
        ...
        );
        {
            ...
            if queue_backpressure_reject(&mut inner, "outbound_queue_backpressure_sync") {
                if fail_on_backpressure {
                    return Err(PulseError::Internal(format!(
                        "{kind} rejected by outbound queue backpressure"
                    )));
                }
                return Ok(());
            }
        ...
        if let Err(error) = self.outbound_tx.send(msg) {
            if let Ok(mut inner) = self.inner.lock() {
                ...
                if block_lane {
                    ...
                    inner.queued_non_block_messages =
                        inner.queued_non_block_messages.saturating_sub(1);
                }
            }
    ...
    fn queue_sync_message(&self, msg: OutboundMessage, kind: &str) -> Result<(), PulseError> {
        self.queue_sync_message_with_policy(msg, kind, false)
    }

    fn queue_sync_message_required(
        &self,
        msg: OutboundMessage,
        kind: &str,
    ) -> Result<(), PulseError> {
        self.queue_sync_message_with_policy(msg, kind, true)
    }
}
...
impl P2pHandle for Libp2pHandle {
    ...
    fn submit_reconstructed_compact_block_v1(
        ...
        self.queue_sync_message_required(
            ...
            "compact-relay-reconstructed-block",
        )
    }
    ...
    fn send_compact_relay_full_block_fallback_v1(
        ...
        self.queue_sync_message_required(
            ...
            "compact-relay-full-block-fallback",
        )
    }
    ...
    fn request_block_from(
        ...
        let request_id = format!("getblock:{peer_id}:{hash}:{sent_at}");
        {
            ...
            inner.getblock_requests_sent_total =
                inner.getblock_requests_sent_total.saturating_add(1);
            inner.peer_addressed_getblock_sent_total =
                inner.peer_addressed_getblock_sent_total.saturating_add(1);
            inner.outstanding_getblock_requests.insert(
                request_id.clone(),
        ...
        if let Err(error) = self.queue_sync_message_required(
            OutboundMessage::GetBlockFrom {
                peer_id: peer_id.to_string(),
                hash: hash.clone(),
                request_id: request_id.clone(),
            ...
            if let Ok(mut inner) = self.inner.lock() {
                inner.outstanding_getblock_requests.remove(&request_id);
                inner.getblock_requests_sent_total =
                    inner.getblock_requests_sent_total.saturating_sub(1);
                inner.peer_addressed_getblock_sent_total =
                    inner.peer_addressed_getblock_sent_total.saturating_sub(1);
                inner.peer_addressed_getblock_transport_error_total = inner
                    .peer_addressed_getblock_transport_error_total
                    .saturating_add(1);
            }
...
mod tests {
    ...
    #[test]
    fn strict_sync_enqueue_reports_backpressure_without_sending() {
        ...
        let error = handle
            .queue_sync_message_required(OutboundMessage::GetTips, "strict-test")
            .expect_err("strict enqueue must report backpressure");
        assert!(error
            .to_string()
            .contains("strict-test rejected by outbound queue backpressure"));
        assert!(outbound_rx.try_recv().is_err());

        let guard = handle.inner.lock().unwrap();
        assert_eq!(guard.queue_backpressure_drops, 1);
        assert_eq!(
            guard.last_drop_reason.as_deref(),
            Some("outbound_queue_backpressure_sync")
        );
    }

    #[test]
    fn strict_sync_enqueue_rolls_back_depth_if_outbound_channel_is_closed() {
        let (outbound_tx, outbound_rx) = mpsc::unbounded_channel::<OutboundMessage>();
        drop(outbound_rx);
        let handle = Libp2pHandle {
        ...
        handle
            .queue_sync_message_required(OutboundMessage::GetTips, "closed-channel-test")
            .expect_err("closed outbound channel must fail");
    ...
    #[test]
    fn compact_full_block_fallback_enqueues_canonical_block_data() {
        let (outbound_tx, mut outbound_rx) = mpsc::unbounded_channel::<OutboundMessage>();
        let handle = Libp2pHandle {
            inner: Arc::new(Mutex::new(InnerState::default())),
            outbound_tx,
        };
        let block = sample_block("compact-full-fallback", 7);
        handle
            .send_compact_relay_full_block_fallback_v1(&block.hash, Some(&block))
            .expect("compact full-block fallback enqueue");

        assert!(matches!(
            outbound_rx.try_recv(),
            Ok(OutboundMessage::BlockData {
                block: Some(queued),
                request_id: None,
                request_hash: Some(request_hash),
            }) if queued.hash == block.hash && request_hash == block.hash
        ));
    }
...
mod inventory_tests {
    ...
    #[test]
    fn full_block_fallback_and_new_block_share_canonical_shape_validation() {
        let invalid = Block {
            hash: "00".repeat(32),
            header: pulsedag_core::types::BlockHeader {
                version: 1,
        ...
        let new_block_inner = Arc::new(Mutex::new(InnerState::default()));
        let (new_block_tx, mut new_block_rx) = mpsc::unbounded_channel();
        let new_block_wire = serde_json::to_vec(&NetworkMessage::NewBlock {
            chain_id: "testnet".into(),
            block: invalid.clone(),
        })
        .expect("serialize invalid new block");
        dispatch_network_message(
            "testnet",
            &new_block_wire,
            Some("peer-a"),
        ...
        let fallback_inner = Arc::new(Mutex::new(InnerState::default()));
        let (fallback_tx, mut fallback_rx) = mpsc::unbounded_channel();
        let fallback_wire = serde_json::to_vec(&NetworkMessage::BlockData {
            chain_id: "testnet".into(),
            block: Some(invalid.clone()),
            request_id: None,
            request_hash: Some(invalid.hash.clone()),
        })
        .expect("serialize invalid fallback block data");
        dispatch_network_message(
            "testnet",
            &fallback_wire,
            Some("peer-a"),
        ...
        let new_block_guard = new_block_inner.lock().unwrap();
        let fallback_guard = fallback_inner.lock().unwrap();
        assert_eq!(new_block_guard.invalid_blocks_received, 1);
        assert_eq!(fallback_guard.invalid_blocks_received, 1);
        assert_eq!(
            new_block_guard.last_drop_reason.as_deref(),
            Some("invalid_block_block_hash_mismatch")
        );
        assert_eq!(
            fallback_guard.last_drop_reason.as_deref(),
            Some("invalid_block_data_block_hash_mismatch")
        );
    }
...
mod deterministic_p2p_sync_coverage_tests {
    ...
    #[test]
    fn zero_peer_success_requires_pending_attempt() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        ...
        let guard = inner.lock().unwrap();
        assert_eq!(guard.peer_zero_reconnect_attempt_total, 0);
        assert_eq!(guard.peer_zero_reconnect_success_total, 0);
    }

    #[test]
    fn zero_peer_attempt_increments_when_isolated_with_bootnode() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        record_bootnode_reconnect_schedule(&inner, &peer, 10_000);

        let guard = inner.lock().unwrap();
        assert_eq!(guard.peer_zero_reconnect_attempt_total, 1);
        assert!(guard
            .peer_zero_reconnect_pending
            .contains(&peer.to_string()));
        assert_eq!(guard.peer_zero_reconnect_success_total, 0);
    ...
    #[test]
    fn zero_peer_dial_failure_clears_pending_without_success() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        state.peer_zero_reconnect_pending.insert(peer.to_string());
        state.pending_bootnode_dials.insert(peer.to_string());
        let inner = Arc::new(Mutex::new(state));
        let mut pending = HashSet::new();
        pending.insert(peer);

        assert!(handle_outgoing_connection_error(
            &inner,
            &mut pending,
            &peer,
            "dial failed"
        ...
        assert_eq!(guard.peer_zero_reconnect_success_total, 0);
        assert_eq!(
            guard.last_peer_reconnect_blocked_reason.as_deref(),
            Some("dial_failed")
        );
    }

    #[test]
    fn peer_zero_transient_connection_established_waits_for_fresh_status_success() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        state.peer_zero_reconnect_pending.insert(peer.to_string());
        state.peer_zero_reconnect_attempt_total = 1;
        let inner = Arc::new(Mutex::new(state));
        let mut pending = HashSet::new();
        ...
        inner
            ...
            .or_default()
            .connected = true;
        enforce_connectivity_aware_cooldown_floor(&mut inner.lock().unwrap(), 10_010);

        let guard = inner.lock().unwrap();
        assert_eq!(guard.peer_zero_reconnect_success_total, 1);
        assert!(!guard
            .peer_zero_reconnect_pending
            .contains(&peer.to_string()));
    }
    ...
    #[test]
    fn peer_zero_reconnect_pending_keeps_redial_active_without_duplicate_attempt() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        let inner = Arc::new(Mutex::new(state));
        record_bootnode_reconnect_schedule(&inner, &peer, 10_000);
        record_bootnode_reconnect_schedule(&inner, &peer, 10_001);
        assert_eq!(
            inner.lock().unwrap().peer_zero_reconnect_attempt_total,
            1,
            "pending zero-peer recovery must not count duplicate attempts"
        );
    }

    #[test]
    fn peer_zero_keepalive_timeout_while_isolated_rearms_reconnect() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        assert!(handle_connection_closed(
            ...
            &mut backoff,
            &peer,
            "swarm-connection-closed:Some(KeepAliveTimeout)".into(),
            "outbound"
        ));

        let guard = inner.lock().unwrap();
        let scheduled = guard.bootnode_next_redial_at[&peer.to_string()];
        assert!(scheduled <= now_unix());
        assert!(guard.peer_zero_since_unix.is_some());
        assert!(guard
            .peer_zero_reconnect_pending
            .contains(&peer.to_string()));
        assert_eq!(guard.peer_zero_reconnect_success_total, 0);
        assert!(next_redial.get(&peer).copied().unwrap_or(u64::MAX) <= now_unix());
    }

    #[test]
    fn peer_zero_configured_bootnode_schedules_real_reconnect_due() {
        let key = identity::Keypair::generate_ed25519();
        let peer = PeerId::from(key.public());
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
        ...
        enforce_connectivity_aware_cooldown_floor(&mut state, 42);
    ...
    #[test]
    fn zero_peer_without_configured_peers_sets_blocked_reason() {
        let mut state = InnerState {
            mode: P2P_MODE_LIBP2P_REAL.into(),
            ..InnerState::default()
        };
        ...
        assert_eq!(state.peer_zero_reconnect_attempt_total, 0);
        assert_eq!(
    ...
    fn tip_inventory_for_test(generation: u64, observed_at_unix: u64) -> TipInventoryStatus {
        TipInventoryStatus {
    ...
    fn remote_tip_inventory_replaces_by_generation_then_freshness() {
        ...
        state.active_connections.insert("peer-a".to_string(), 1);
    ...
    #[test]
    fn remote_tip_inventory_prune_removes_disconnected_expired_and_wrong_chain() {
        let mut state = InnerState {
            chain_id: "test-chain".to_string(),
            ..InnerState::default()
        };
        state.active_connections.insert("connected".to_string(), 1);
        state.active_connections.insert("expired".to_string(), 1);
        assert!(note_remote_tip_inventory(
            &mut state,
            "connected",
