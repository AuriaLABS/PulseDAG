...
pub struct P2pStatus {
    ...
    pub dropped_inbound_messages: usize,
...
struct InnerState {
    ...
    dropped_inbound_messages: usize,
...
impl P2pHandle for MemoryP2pHandle {
    ...
    fn status(&self) -> Result<P2pStatus, PulseError> {
        ...
        Ok(P2pStatus {
            ...
            dropped_inbound_messages: inner.dropped_inbound_messages,
...
fn note_inbound_drop(inner: &Arc<Mutex<InnerState>>, reason: &str) {
    if let Ok(mut guard) = inner.lock() {
        guard.dropped_inbound_messages = guard.dropped_inbound_messages.saturating_add(1);
        guard.last_swarm_event = Some(format!("inbound-drop:{reason}"));
    }
}
    ...
    match msg {
        NetworkMessage::NewTransaction {
            ...
            if inbound_tx
                ...
                .is_err()
            {
                note_inbound_drop(inner, "tx-channel-full");
            }
        }
        NetworkMessage::NewBlock { chain_id, block } => {
            ...
            if inbound_tx.try_send(InboundEvent::Block(block)).is_err() {
                note_inbound_drop(inner, "block-channel-full");
            }
        }
        ...
        NetworkMessage::BlockData { chain_id, block } => {
            ...
            if let Some(block) = block {
                ...
                if inbound_tx.try_send(InboundEvent::Block(block)).is_err() {
                    note_inbound_drop(inner, "block-data-channel-full");
                }
            }
        }
    }
...
fn fake_swarm_bootstrap_events(
    ...
    if inbound_tx
        .try_send(InboundEvent::PeerConnected(format!(
            "local:{}",
            local_peer_id
        )))
        .is_err()
    {
        note_inbound_drop(inner, "peer-connected-local-channel-full");
    }
    note_swarm_event(inner, "listen-started");
    for addr in &cfg.bootstrap {
        if inbound_tx
            .try_send(InboundEvent::PeerConnected(addr.clone()))
            .is_err()
        {
            note_inbound_drop(inner, "peer-connected-bootstrap-channel-full");
        }
