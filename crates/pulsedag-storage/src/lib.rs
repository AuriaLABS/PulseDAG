...
impl Storage {
    fn actionable_delta_from_snapshot(snapshot: &ChainState, blocks: &[Block]) -> Vec<Block> {
        ...
        blocks
            ...
            .filter(|block| {
                block.header.height > snapshot_height
                    ...
                    && !snapshot.dag.blocks.contains_key(&block.hash)
            })
            .cloned()
            .collect()
    }
    ...
    pub fn replay_blocks_or_init_with_report(
        ...
        if let Ok(Some(snapshot)) = snapshot {
            let delta_blocks = Self::actionable_delta_from_snapshot(&snapshot, &blocks);
            match rebuild_state_from_snapshot_and_blocks(snapshot, delta_blocks) {
                Ok(state) => {
    ...
    pub fn replay_from_validated_snapshot_and_delta(&self) -> Result<ChainState, PulseError> {
        ...
        let blocks = self.list_blocks()?;
        let delta_blocks = Self::actionable_delta_from_snapshot(&snapshot, &blocks);
        let state = rebuild_state_from_snapshot_and_blocks(snapshot, delta_blocks)?;
        self.persist_chain_state(&state)?;
        Ok(state)
    }
    ...
    pub fn restore_drill_snapshot_and_delta(
        ...
        let (state, used_snapshot, fallback_to_full_rebuild) = match snapshot {
            Ok(Some(snapshot_state)) => {
                let delta_blocks =
                    Self::actionable_delta_from_snapshot(&snapshot_state, &persisted_blocks);
                match rebuild_state_from_snapshot_and_blocks(snapshot_state, delta_blocks) {
