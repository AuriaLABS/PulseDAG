...
fn retarget_is_identical_after_restart_pruning_delta_replay_and_second_node_replay() {
    ...
    let restart_bytes = bincode::serialize(&canonical).expect("canonical state should serialize");
    let restarted: ChainState =
        bincode::deserialize(&restart_bytes).expect("canonical state should deserialize");
    assert_consensus_agreement("binary storage restart", &canonical, &restarted);
